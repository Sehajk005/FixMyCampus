  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2rem 1.5rem 6rem' }}>
      ...
      <div className="animate-fade-up" style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: 'var(--text-muted)', marginTop: '0.25rem' }}>Report campus issues and track them in real-time.</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '2rem' }}>
        {[
          ...
          <div key={s.label} className={`stat-card animate-fade-up ${s.delay}`}>
            ...
            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.4rem', fontWeight: 500 }}>{s.label}</div>
          </div>
        ))}
      </div>

      {/* CTA */}
      <div className="animate-fade-up delay-300" style={{ background: 'linear-gradient(135deg, color-mix(in oklab, var(--accent) 20%, transparent), color-mix(in oklab, var(--accent2) 14%, transparent))', border: '1px solid color-mix(in oklab, var(--accent) 35%, transparent)', borderRadius: '1rem', padding: '1.75rem', marginBottom: '2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
        <div>
          <h2 style={{ fontWeight: 700, fontSize: '1.1rem', color: 'var(--text)', marginBottom: '0.3rem' }}>Spotted an issue?</h2>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>Report it now and our team will get on it.</p>
        </div>
      ...
      <div className="animate-fade-up delay-400">
        <h2 style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '1rem' }}>Quick Report by Category</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.75rem' }}>
          {CATS.map((c, i) => (
            <Link key={c.v} to={`/tickets/new?category=${c.v}`} className={`animate-fade-up delay-${(i + 1) * 100}`}
              style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', padding: '1.25rem 0.75rem', borderRadius: '0.875rem', background: 'var(--surface)', border: `1px solid var(--border)`, cursor: 'pointer', textDecoration: 'none', transition: 'all 0.25s' }}
      ...
      {tickets.length > 0 && (
        <div className="animate-fade-up delay-500" style={{ marginTop: '2rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <h2 style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Recent Reports</h2>
            <Link to="/tickets" style={{ fontSize: '0.8rem', color: 'var(--accent)', textDecoration: 'none', fontWeight: 600 }}>View all →</Link>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
            {tickets.slice(0, 3).map((t, i) => (
              <Link key={t.id} to={`/tickets/${t.id}`} className={`animate-fade-up delay-${(i + 1) * 100}`}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.25rem', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '0.875rem', textDecoration: 'none', transition: 'all 0.2s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'color-mix(in oklab, var(--accent) 40%, transparent)'; e.currentTarget.style.background = 'color-mix(in oklab, var(--accent) 10%, var(--surface))'; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.background = 'var(--surface)'; }}
              >
                <div>
                  <p style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text)', marginBottom: '0.2rem' }}>{t.title}</p>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'capitalize' }}>{t.status?.replace('_', ' ')}</span>
                </div>
              </Link>
            ))}
          </div>
